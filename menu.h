...
namespace ZeroGUI
{
	...
	void Draw_Cursor(bool toogle)
	{
		...
		{
			Vector2 cursorPos = CursorPos();
			canvas->K2_DrawLine(Vector2{ cursorPos.x, cursorPos.y }, Vector2{ cursorPos.x + 35, cursorPos.y + 10 }, 1, ue::FLinearColor{ 0.0f, 0.0f, 0.0f, 1.00f });
			...
			canvas->K2_DrawLine(Vector2{ cursorPos.x, cursorPos.y }, Vector2{ cursorPos.x + 15, cursorPos.y + 30 }, 1, ue::FLinearColor{ 0.0f, 0.0f, 0.0f, 1.00f });
			canvas->K2_DrawLine(Vector2{ cursorPos.x + 35, cursorPos.y + 10 }, Vector2{ cursorPos.x + 15, cursorPos.y + 30 }, 1, ue::FLinearColor{ 0.0f, 0.0f, 0.0f, 1.00f });
		}
	}
	...
	static void SliderFloat(const char* name, float* value, float min, float max, const char* format = "%.0f")
	{
		...
		char buffer[32];
		sprintf_s(buffer, format, *value);
		Vector2 valuePos = Vector2{ pos.x + slider_size.x + 30, pos.y + 20};
		TextCenter(buffer, valuePos, Colors::Text, false, false);
...
namespace Settings
{
	...
	bool LinesESP = true;
	bool PlayerNameESP = true;
	bool DistanceESP = true;
	bool PlatformESP = true;
	bool EnemyWeaponESP = false;
	bool chestesp = false;
	bool ammobox = false;
	bool vehicleespnerds = false;
	bool LootESP = false;
	bool ProjectileESP = false;
	bool ProjectileObjectNameESP = false;
	bool DMRSeller = false;
	bool ActorObjectName = false;

	//Mods Tab
	bool FovChanger = false;
	float FovChanger_Value = 125;
	bool SniperTp = true;
	bool RapidFire = false;
	bool FullAutoWeapons = true;
	bool CurrentWeaponSpeed = true;
	bool NoRecoil = true;
	bool InstantReload = true;
	bool NoWeaponCoolDown = true;
	bool NoSpread = false;
	bool ProjectileDestroyer = false;
	bool MagicBullet = true;
	bool VehicleFly = true;
	float VehicleFlySpeed = 200;
	bool SyncCameraRotation = true;
	bool VehicleTP = false;
	bool VehicleMapMarkerToTeleport = true;
	Vector3 MarkerLocation;
	bool VehicleNoGravity = true;
	bool PlayerFly = false;
	float PlayerFlySpeed = 1000;
	Vector3 SetLocation;
	Vector3 AddPlayerFlyLocation;
	bool DisableGetCurrentLocation = false;
	bool ZiplineTeleport = true;
	bool LocalPawnSpeed = false;
	bool QuickFly = true;
	bool Speedhack = false;
	float SpeedhackValue = 50;
	bool UnrealEngineSpeedhack = true;
	float UnrealEngineSpeedhackValue = 50;

	float MemoryAimSmooth = 0;
	float worldespdistance = 300;
	bool AllExploits = true;
	bool LevelESP = true;
	bool KillsESP = true;
}
